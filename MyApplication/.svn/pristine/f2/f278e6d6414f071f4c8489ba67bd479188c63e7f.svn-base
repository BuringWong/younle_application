package com.yongle.letuiweipad.utils.ticketprinter;
...
public class DeviceConnFactoryManager {
    ...
    private static final String TAG = "DeviceConnFactoryManage";
    ...
    private void queryPrinterCommand() {
        ThreadPool.getInstantiation().addTask(new Runnable() {
            @Override
            public void run() {
                LogUtils.e(TAG,"start query mode");
                //发送ESC查询打印机状态指令
//                sendCommand = esc;
                sendCommand = tsc;
...
//                            sendCommand = tsc;
            ...
            switch (msg.what) {
                case READ_DATA:
                    ...
                    if (sendCommand == esc) {
                        ...
                        LogUtils.e(TAG,"current printmode is tsc!!");
                        //设置当前打印机模式为TSC模式
                        if (currentPrinterCommand == null) {
    ...
    public void removeReceiver(Context context){
        LogUtils.e(TAG,"removeReceiver");
        try{
            if(usbStateReceiver!=null) {
                context.unregisterReceiver(usbStateReceiver);
            }
        }catch (Exception e){
            LogUtils.e(TAG,"解绑断开receiver异常"+e.toString());
        }
