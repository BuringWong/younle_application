...
import com.younle.younle624.myapplication.domain.YunPrintBean;
...
import static com.younle.younle624.myapplication.constant.Constant.FOR_MERCHANT;
import static com.younle.younle624.myapplication.constant.Constant.STORE_TEL;
import static com.younle.younle624.myapplication.constant.Constant.numDf;
import static com.younle.younle624.myapplication.utils.Utils.df4;
...
public class YunPrintUtils {
    ...
    public static void printBillDetailNormalOrder(Context context,Map<String,List<WmPintData>> map){
        ...
        List<WmPintData> mData = map.get(Constant.FOR_MERCHANT);
        ...
        printData.put(Constant.FOR_MERCHANT,mstr);
     ...
     */
    public static List<YunPrintGroupBean> formateSplitOrderDetailData( List<String> noGroupData, UnPayDetailsBean bean){
        ...
        formatOneByOneYunPrintData(bean,list);
        return list;
     ...
     * @param printList
     */
    public static  void formatOneByOneYunPrintData(UnPayDetailsBean orderBean, List<YunPrintGroupBean> printList){
        //3.添加数据
        List<UnPayDetailsBean.MsgBean.OrderGoodsBean> goodList = orderBean.getMsg().getOrder_goods();
        if(goodList!=null&&goodList.size()>0) {
            for (int i = 0; i < goodList.size() ; i++) {
                if(TextUtils.equals("0",goodList.get(i).getId())) {
                    continue;
                }
                String printMsg="";
                printMsg+= typefacesize3(goodList.get(i).getGoods_name())+"\n";
                ...
                printMsg+= typefacesize3("数量:"+Utils.dropZero(goodList.get(i).getGoods_num()+""))+"\n";

                printMsg=addOrderOrRoomDesk(printMsg,orderBean.getMsg().getOrder_rooms());
                if(!"".equals(Constant.CURRENT_ORDER_ROOM)) {
                    printMsg+="订单序号：" + Constant.CURRENT_ORDER_NUM+"\n";
                }
    ...
    public static  void formatOneByOneYunPrintData(AppLetReceiveBean orderBean, List<YunPrintGroupBean> printList){
        ...
        if(goodList!=null&&goodList.size()>0) {
            for (int i = 0; i < goodList.size() ; i++) {
                ...
                if(orderBean.getRoomName()!=null&&!TextUtils.isEmpty(orderBean.getRoomName())) {
                    printMsg+="订单序号："+orderBean.getDayOrderNum()+"\n";
                }
    ...
    public static  void formatOneByOneYunPrintData(PosPrintBean orderBean, List<YunPrintGroupBean> printList){
        ...
        if(goodList!=null&&goodList.size()>0) {
            for (int i = 0; i < goodList.size() ; i++) {
                ...
                printMsg += typefacesize3(orderNo) + "\n";
                if(orderBean.getMsg().getRoomName()!=null&&!TextUtils.isEmpty(orderBean.getMsg().getRoomName())) {
                    printMsg+="订单序号："+orderBean.getMsg().getDayOrderNum()+"\n";
                }
    ...
    public static void formateWMOrderData(Context context, Map printMap){
        List<WmPintData> merchant = (List<WmPintData>) printMap.get(Constant.FOR_MERCHANT);
        ...
        map.put(Constant.FOR_MERCHANT,merchantStr);
    ...
    public static Map<String,List<WmPintData>> formatPosDetailData(PosPrintBean posBean, DiscountInfo discountBean) {
        ...
        map.put(Constant.FOR_MERCHANT,mData);
        return map;
    }
...
//            headerData.add("\n");
    ...
    public static void addBottom(List<WmPintData> data) {
    ...
    private static void posDetailCommonPartForApplet(List<WmPintData> data, PosPrintBean posBean, boolean needPhone, DiscountInfo discountBean) {
        ...
        if(posBean.getMsg().getRoomName()!=null&&!"".equals(posBean.getMsg().getRoomName())){
            data.add(new WmPintData("订单序号:" + posBean.getMsg().getDayOrderNum(),size27,WmPintData.LEFT));
        }
...
//        data.add(new WmPintData("实付金额:" + posBean.getMsg().getFactPayMoney() + "元", size27, WmPintData.LEFT));
    ...
    private static void posDetailCommonPartForCommunity(List<WmPintData> data, PosPrintBean posBean, boolean needPhone, DiscountInfo discountBean) {
        data.add(new WmPintData(Constant.STORE_P + Constant.STORE_M,size27,WmPintData.CENTER));
        if(needPhone) {
            data.add(new WmPintData("电话:" + Constant.STORE_TEL,size27,WmPintData.CENTER));
        }
...
//        data.add(new WmPintData("实付金额:" + posBean.getMsg().getFactPayMoney() + "元", size27, WmPintData.LEFT));
    ...
    public static Map formatYunPrintDataWM(MtOrderDetail mtOrder, ElmOrderBean elmBean, NewElmOrderBean newElmOrderBean, Context context, boolean isbt){
        ...
        map.put(Constant.FOR_MERCHANT,merchantPart);
        return map;
    }
...
//        pData.add("\n");
     ...
     * @return
     */
    public static List<YunPrintGroupBean> formatPrePrintData(UnPayDetailsBean posPrintBean,boolean normalMerchant){
        ArrayList<YunPrintGroupBean> oneByOneYunList=new ArrayList<>();
        LogUtils.Log("添加打印数据：formatPrintData()");
        formatPreNormal(posPrintBean,oneByOneYunList,true);

        if(normalMerchant) {
            LogUtils.Log("添加顾客联后："+oneByOneYunList.toString());
        }
        //商家联
        if(normalMerchant) {
            formatPreNormal(posPrintBean,oneByOneYunList,false);
            LogUtils.Log("添加商家联后："+oneByOneYunList);
        }else {
            formatOneByOneYunPrintData(posPrintBean,oneByOneYunList);
        }
        return oneByOneYunList;
    }
    public static void formatPreNormal(UnPayDetailsBean orderBean,ArrayList<YunPrintGroupBean> oneByOneYunList,boolean customer){
        textSize=27;
        //来自第一次提交到总订单，并且不是只有房间
        List<WmPintData> headData=new ArrayList<>();
        ...
        if ((order_rooms != null && order_rooms.size() > 0) && (order_entity == null || order_entity.size() <= 0)) {
            ...
            if(customer) {
                ...
                String yunLineData = getYunLineData(headData,true,0);
                oneByOneYunList.add(0,new YunPrintGroupBean(yunLineData,Constant.GROUP_CUSTOMER));
            }else {
                String yunLineData = getYunLineData(headData,true,0);
                //添加非商品头部
                oneByOneYunList.add(new YunPrintGroupBean(yunLineData,Constant.MERCHANT_COMMONPART));

                if (order_entity != null && order_entity.size() > 0) {
                    for (int i = 0; i < order_entity.size(); i++) {
                        List<WmPintData> goodData=new ArrayList<>();
                        String name = order_entity.get(i).getGoods_name();
                        String num = Utils.keepTwoDecimal(order_entity.get(i).getGoods_num()+"");
                        double good_price = order_entity.get(i).getGoods_price();
                        double goods_ori_price = order_entity.get(i).getGoods_ori_price();//原价
                        double total_fee = order_entity.get(i).getTotal_fee();
                        addFormatWmTest(goodData, name, Utils.keepTwoDecimal(good_price+""),num, Utils.keepTwoDecimal(total_fee+""),size27);
                        if(goods_ori_price -good_price>0) {
                            goodData.add(new WmPintData("原单价:"+goods_ori_price+"元,会员优惠:"+ Utils.dropZero(Utils.keepTwoDecimal(goods_ori_price-good_price+""))+"元"));
                        }
                        String goodLineData = getYunLineData(goodData,true,0);
                        oneByOneYunList.add(new YunPrintGroupBean(goodLineData,order_entity.get(i).getGroup_id()));
                    }
                }
                String footer="";
                footer+=line27+"\n";
                footer+="商家联"+"\n";
                oneByOneYunList.add(new YunPrintGroupBean(footer,Constant.MERCHANT_COMMONPART));
            }
        }
    }
    /**
     ...
     * @return
     */
    public static List<YunPrintGroupBean> formatFinishedAppletYunPrintData(PosPrintBean posPrintBean,DiscountInfo discountInfo, Context context,boolean normalMerchant){
        ArrayList<YunPrintGroupBean> oneByOneYunList=new ArrayList<>();
        LogUtils.Log("添加打印数据：formatPrintData()");
        List<WmPintData> yunPrintData = new ArrayList<>();
        posFinishedAppletForCustomer(yunPrintData,posPrintBean);
        if(discountInfo!=null) {
            totalPrintMemberTest(discountInfo, yunPrintData);
        }else{
            wmCenterFormat("-","-", yunPrintData,size27);
        }
        if(Constant.bottomData!=null&&Constant.bottomData.length>0) {
            for (int i = 0; i < Constant.bottomData.length; i++) {
                yunPrintData.add(new WmPintData(size27,Constant.bottomData[i]));
            }
        }
        ...
        //商家联
        if(normalMerchant) {
            formaFinishedNormalMerchant(posPrintBean,oneByOneYunList);
        }else {
            formatOneByOneYunPrintData(posPrintBean,oneByOneYunList);
        }
        return oneByOneYunList;
     ...
     * @return
     */
    public static List<YunPrintGroupBean> formatAppletYunPrintData(AppLetReceiveBean appLetBean, Context context,boolean normalMerchant){
        ArrayList<YunPrintGroupBean> oneByOneYunList=new ArrayList<>();
        LogUtils.Log("添加打印数据：formatPrintData()");
        List<WmPintData> yunPrintData = new ArrayList<>();
        ...
        //商家联
        if(normalMerchant) {
            formaNormalMerchant(appLetBean,oneByOneYunList);
        }else {
    ...
    private static void  formaNormalMerchant(AppLetReceiveBean appLetBean, ArrayList<YunPrintGroupBean> oneByOneYunList) {
        List<WmPintData> data = new ArrayList<>();
        data.add(new WmPintData(Constant.STORE_P + Constant.STORE_M, size27, WmPintData.CENTER));
        if(appLetBean.getRoomName()!=null&&!"".equals(appLetBean.getRoomName())){
            data.add(new WmPintData(appLetBean.getRoomName(), size50, WmPintData.CENTER));
        }else{
            data.add(new WmPintData("订单序号:" + appLetBean.getDayOrderNum(), size50, WmPintData.CENTER));
        }
        if(appLetBean.getOrderType()==4) {
            if(appLetBean.getDeliveryStatus()==0||appLetBean.getDeliveryStatus()==1) {
                ...
                centerFormat("顾客信息","-",data, size27);
                data.add(new WmPintData(size50,appLetBean.getCneeAddress()));
                data.add(new WmPintData("姓名："+appLetBean.getCneeName()));
                data.add(new WmPintData("手机号："+appLetBean.getCneePhone()));
            }else {
                data.add(new WmPintData("自提",size50,WmPintData.CENTER));
                centerFormat("顾客信息","-",data, size27);
                data.add(new WmPintData(size50,"自提码："+appLetBean.getZt_code()));
            ...
            if(appLetBean.getRoomName()!=null&&!"".equals(appLetBean.getRoomName())){
                data.add(new WmPintData("订单序号:" + appLetBean.getDayOrderNum()));
            }
...
//            }
        ...
        if(detailAppletContainWhich(goodsInfo,"2")||detailAppletContainWhich(goodsInfo,"1")||detailAppletContainWhich(goodsInfo,"0")) {
            ...
            //单纯添加商品
            for (int i = 0; i < goodsInfo.size(); i++) {
                List<WmPintData> goods = new ArrayList<>();
                ...
                if("0".equals(goodsInfoBean.getType())||"1".equals(goodsInfoBean.getType())||"2".equals(goodsInfoBean.getType())) {
                    ...
                    addFormatWmTest(goods, goodsInfoBean.getName(), Utils.keepTwoDecimal(price+""),
                            Utils.keepTwoDecimal(goodsInfoBean.getNum()), numDf.format(Double.valueOf(goodsInfoBean.getNum()) * price),size27);

                    if(is_vip==1&&ori_price-goodsInfoBean.getPrice()>0) {
                        goods.add(new WmPintData("原单价:"+Utils.keepTwoDecimal(ori_price+"")+",会员优惠:"+Utils.keepTwoDecimal(ori_price-price+"")));
                    }
                    String goodLineData = getYunLineData(goods,true,0);
                    oneByOneYunList.add(new YunPrintGroupBean(goodLineData,goodsInfoBean.getGroup_id()));
                }
            }
        }
        //添加公共底部数据
    ...
    private static void formaFinishedNormalMerchant(PosPrintBean posPrintBean, ArrayList<YunPrintGroupBean> oneByOneYunList) {
        ...
        if(appLetBean.getOrderType()==3) {
            if(appLetBean.getDeliveryStatus()==0||appLetBean.getDeliveryStatus()==1) {
                ...
                data.add(new WmPintData("自提",size50,WmPintData.CENTER));
                centerFormat("顾客信息","-",data, size27);
                data.add(new WmPintData(size50,"自提码："+appLetBean.getOrderid()));
                data.add(new WmPintData("自提时间："+appLetBean.getDeliveryTime()));
                data.add(new WmPintData("手机号："+appLetBean.getCneePhone()));
            }
        }else {
    ...
    private static void posAppletForCustomer(List<WmPintData> data, AppLetReceiveBean appLetBean) {
        ...
        if(appLetBean.getOrderType()==4) {
            if(appLetBean.getDeliveryStatus()==0||appLetBean.getDeliveryStatus()==1) {
                ...
                data.add(new WmPintData(appLetBean.getDeliveryTime(),size27,WmPintData.CENTER));
                centerFormat("顾客信息","-",data, size27);
                data.add(new WmPintData(appLetBean.getCneeAddress(),size50,WmPintData.LEFT));
                data.add(new WmPintData("姓名："+appLetBean.getCneeName()));
                data.add(new WmPintData("手机号："+appLetBean.getCneePhone()));
            }else {
                data.add(new WmPintData("自提",size50,WmPintData.CENTER));
...
//            }
        ...
        if(detailAppletContainWhich(goodsInfo,"2")||detailAppletContainWhich(goodsInfo,"1")||detailAppletContainWhich(goodsInfo,"0")) {
            ...
            for (int i = 0; i < goodsInfo.size(); i++) {
                ...
                if("0".equals(goodsInfoBean.getType())||"1".equals(goodsInfoBean.getType())||"2".equals(goodsInfoBean.getType())) {
                    ...
                    double price = goodsInfoBean.getPrice();
                    int is_vip = goodsInfoBean.getIs_vip();
                    addFormatWmTest(data, goodsInfoBean.getName(), Utils.keepTwoDecimal(price+""),
                            Utils.keepTwoDecimal(goodsInfoBean.getNum()), numDf.format(Double.valueOf(goodsInfoBean.getNum()) * price),size27);
     ...
     * @param data
     */
    private static void posFinishedAppletForCustomer(List<WmPintData> data, PosPrintBean posPrintBean) {
...
//            }
            ...
            case 0://顾客联
                for(int i=0;i<headerData.size();i++){
                    if(judgetosideLine(headerData.get(i))){
                        ...
                        if(headerData.get(i).contains("原单价")) {
                            yunPrintData += headerData.get(i)+"\n\n";
                        }else {
                            yunPrintData += headerData.get(i)+"\n";
                        }
                    }
                ...
                //添加底部打印信息
                yunPrintData = addBottomData(yunPrintData);
                yunPrintData += line32;
                yunPrintData += "顾客联\n";
                break;
            case 1://商家联
    ...
    private static String addBottomData(String yunPrintData) {
        if(Constant.bottomData!=null&&Constant.bottomData.length>0) {
            for (int i = 0; i < Constant.bottomData.length; i++) {
                yunPrintData+=Constant.bottomData[i]+"\n";
            }
        }
        return yunPrintData;
    }
    ...
    public static void YunToPrintNew(Context context, Map<String,String> printMap,String customer,String merchant){
        ...
        if(savedPrinterses !=null && savedPrinterses.size()>0){
            for(int i = 0; i< savedPrinterses.size(); i++){
                ...
                yun_print_list.add(yunPrintBean);
                printer_json = new Gson().toJson(yun_print_list);
                LogUtils.Log("云打印bean:"+yunPrintBean.toString());
     ...
     */
    public static void yunPrintOneByOneNew(Context context,List<SavedPrinter> savedPrinterses, List<YunPrintGroupBean> oneByOneList,List<YunPrintGroupBean> normalList,
        ...
        for(int i = 0; i< savedPrinterses.size(); i++){
            ...
            List<String> printGroupId = savedPrinterses.get(i).getPrintGroupId();
            boolean oneByOne=false;
            switch (customer) {
                case "3" ://预结单
                    oneByOne=savedPrinterses.get(i).isPreobo();
                    break;
                case "9"://自助点单小程序
                    oneByOne=savedPrinterses.get(i).isAppletobo();
                    break;
                case "a"://社区门店小程序
                    oneByOne=savedPrinterses.get(i).isCommunityobo();
                    break;
            }

            if(printGroupId==null) {
                printGroupId=new ArrayList<>();
            }
            if(judeAuto) {//需要先判断是否自动打印
                if(!printPermission.contains("1")) {//不需要自动打印，跳过该打印机
                    ...
                    if(oneByOne) {
                        ...
                        needPrint(normalList, customer, merchant, netWorks, savedPrinterses, i, printPermission, printGroupId,handler);
                    }
                }
            }else {//直接判断是商家联顾客联
                if(oneByOne) {
    ...
    private static void needPrint(List<YunPrintGroupBean> oneByOneList, String customer, String merchant, NetWorks netWorks,
                                  final List<SavedPrinter> savedPrinterses, final int i, String printPermission, List<String> printGroupId, final Handler handler) {
        String printStr="";
        if(printPermission.contains(customer)) {//需要打印顾客联
            if(printPermission.contains(merchant)) {//同时需要打印商家联
                boolean printMerchant=true;
                for (int a = 0; a < oneByOneList.size(); a++) {
                    String groupId = oneByOneList.get(a).getGroupId();
                    if(printGroupId.contains(groupId)||printGroupId.size()<=0) {
                        break;
                    }else {
                        if(a==oneByOneList.size()-1) {
                            printMerchant=false;
                            //商家联没有需要打印的商品不需要打印商家联
                            printStr=oneByOneList.get(0).getMsg();
                        }
                    }
                }
                if(printMerchant) {
                    LogUtils.Log("oneByone:"+oneByOneList.toString());

                    for (int j = 0; j < oneByOneList.size(); j++) {
                        String groupId = oneByOneList.get(j).getGroupId();
                        if(TextUtils.equals(Constant.FOR_CUSTOMER,groupId)
                                ||printGroupId.size()<=0
                                ||printGroupId.contains(groupId)
                                ||TextUtils.equals(Constant.MERCHANT_COMMONPART,groupId)) {
            ...
            if(printPermission.contains(merchant)) {//仅打印商家联
                ...
                for (int j = 1; j <oneByOneList.size() ; j++) {
                    if(printGroupId.size()<=0
                            ||TextUtils.equals(Constant.MERCHANT_COMMONPART,oneByOneList.get(j).getGroupId())
                            ||printGroupId.contains(oneByOneList.get(j).getGroupId())) {
                        printStr+=oneByOneList.get(j).getMsg();
                    }
                }
            }
        ...
        if(!TextUtils.isEmpty(printer_json)) {
            netWorks.YunPrint(1,printer_json,new NetWorks.OnNetCallBack(){
                ...
                public void onError(Exception e, int flag) {
                    LogUtils.Log("云打印接口 flag="+flag+", onError()...Exception e="+e.toString());
                    if(handler!=null) {
                        handler.sendEmptyMessageDelayed(Constant.YUN_PRINT_ERROR,5000);
                    }
