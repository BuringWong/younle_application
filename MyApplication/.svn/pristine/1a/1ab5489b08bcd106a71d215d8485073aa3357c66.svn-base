...
import com.younle.younle624.myapplication.activity.manager.orderpager.bluetoothprinter.bt.BtService;
...
public class BtPrintQueue {
     ...
     * instance
     */
    private static BtPrintQueue mInstance;
    /**
     * context
     */
    ...
    private BtPrintQueue() {
    }

    public static BtPrintQueue getQueue(Context context) {
        if (null == mInstance) {
            mInstance = new BtPrintQueue();
        }
        if (null == mContext) {
            mContext = context;
        }
     ...
     * @param bytes bytes
     */
    public synchronized void add(byte[] bytes,BluetoothService mService) {
        if (null == mQueue) {
        ...
        mService.writeTest(mQueue,null);
    }
    ...
    public synchronized void print() {
        try {
            ...
            if (mBtService.getState() != BtService.STATE_CONNECTED) {
                if (!TextUtils.isEmpty(AppInfo.btAddress)) {//该address从内存中读取
