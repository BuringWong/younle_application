package com.yongle.letuiweipad.utils.scanbar;
...
public class HidConncetUtil {
	...
	public BluetoothProfile.ServiceListener connect = new BluetoothProfile.ServiceListener() {
		...
		public void onServiceConnected(int profile, BluetoothProfile proxy) {
			...
			try {
				...
				// TODO Auto-generated catch block
				// e.printStackTrace();
				LogUtils.e("HidConnectUtils","连接异常："+e.toString());
			}
		}
	...
	public static boolean getScannerConnectState(BluetoothDevice btDev) {
        Class<BluetoothDevice> bluetoothDeviceClass = BluetoothDevice.class;
		try {
			Method getConnectionState = bluetoothDeviceClass.getDeclaredMethod("isConnected");
			boolean connect= (Boolean) getConnectionState.invoke(btDev);
            return connect;
		} catch (Exception e) {
			LogUtils.Log("检查蓝牙连接状态异常=="+e.toString());
			e.printStackTrace();
            return false;
		}
	}
	...
	public static void registBarScanReceiver(Context context,BroadcastReceiver receiver) {
	...
	public static void connectScanner(Context context, String btAdd) {
	...
	/*	scanGunKeyEventHelper=new ScanGunKeyEventHelper(new ScanGunKeyEventHelper.OnScanSuccessListener() {
		...
		HidConncetUtil hidConncetUtil=new HidConncetUtil(context);
		hidConncetUtil.connect(bluetoothAdapter.getRemoteDevice(btAdd));
